...
tailrec fun marchTriangles(
  getVertex: (Int) -> IntermediateMarchingEdge,
  i: Int,
  accumulator: List<MarchingTriangle>
): List<MarchingTriangle> =
...
fun marchingCubes(values: FloatArray, dimensions: Vector3i, isoLevel: Float): List<MarchingTriangle> {
  val result = mutableListOf<MarchingTriangle>()
  for (z in 0 until dimensions.z - 1) {
    for (y in 0 until dimensions.y - 1) {
      for (x in 0 until dimensions.x - 1) {
        ...
        if (bits == 0)
          ...
          fun getVertex(index: Int): IntermediateMarchingEdge {
            val initial = when (index) {
              ...
              5 -> (isoLevel - value5) / (value7 - value5) to (Vector3i(x + 1, y, z + 1) to Vector3i(
                x + 1,
                y + 1,
                z + 1
              ))
              6 -> (isoLevel - value6) / (value7 - value6) to (Vector3i(x, y + 1, z + 1) to Vector3i(
                x + 1,
                y + 1,
                z + 1
              ))
              ...
              10 -> (isoLevel - value3) / (value7 - value3) to (Vector3i(x + 1, y + 1, z) to Vector3i(
                x + 1,
                y + 1,
                z + 1
              ))
          ...
          result += marchTriangles(::getVertex, nextCubeIndex, listOf())
        }
      }
    }
  }
