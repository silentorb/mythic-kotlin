...
import silentorb.mythic.imaging.texturing.filters.allocateFloatBuffer
...
fun bitmapToBufferedImage(bitmap: Bitmap): BufferedImage {
  ...
  for (y in 0 until dimensions.y) {
    for (x in 0 until dimensions.x) {
      for (c in 0 until depth) {
        image.raster.setSample(x, y, c, (buffer.get() * 255f).toInt())
      }
    }
  }
  return image
